...
const readStream = fs.createReadStream("ex02_console.js", {
    encoding: "utf-8", // 인코딩 설정하지 않으면 Buffer로 들어옴. 인코딩 설정하면 string으로 옴
    highWaterMark: 16 * 1024 // 버퍼크기 : 16byte , default : 64 byte
})
// 파일 데이터를 담을 배열
const data = []
...
readStream.on('end', () => {
    ...
    if (typeof data === 'object') { // string으로 들어올 경우
        ...
        // 내부적으로는 byte 단위로 저장되지만, 출력할 때는 사람이 읽기 좋게 16진수(hex)로 표시
    }
})
readStream.on('error', (err) => {
    console.log("파일 읽는 중 에러 발생:", err.message);
});
